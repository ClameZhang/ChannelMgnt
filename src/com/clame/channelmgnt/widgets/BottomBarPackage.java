 ...
 * 
 */
public class BottomBarPackage extends LinearLayout implements OnClickListener {
    ...
    public BottomBarPackage(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        init();
    }
    ...
    public void setSelectedState(int index) {
        if (index != -1 && onPackageItemChangedListener != null) {
            if (index > itemList.size()) {
                throw new RuntimeException("the value of default bar item can not bigger than string array's length");
            }
            itemList.get(index).setSelected(true);
            onPackageItemChangedListener.onPackageItemChanged(index);
            lastButton = index;
        }
    }
    ...
    public interface OnPackageItemChangedListener {
        public void onPackageItemChanged(int index);
    }

    private OnPackageItemChangedListener onPackageItemChangedListener;

    public void setOnPackageItemChangedListener(OnPackageItemChangedListener onPackageItemChangedListener) {
        this.onPackageItemChangedListener = onPackageItemChangedListener;
    }
